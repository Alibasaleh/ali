...
// Theme + local preferences. Works on static hosts (including GitHub Pages).
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const applyMode=(value,save=false)=>{
    const terminal=value==='terminal';
    body.classList.toggle('terminal-mode',terminal);
    body.dataset.theme=terminal?'terminal':'original';
    if(mode){
    ...
    if(meta) meta.content=terminal?'#080b08':'#e9e4d6';
    document.querySelectorAll('.try-terminal').forEach(btn=>{
      btn.innerHTML=terminal?'EXIT TERMINAL <i class="fa-solid fa-arrow-right"></i>':'TRY IT <i class="fa-solid fa-arrow-right"></i>';
    });
    if(save) write(MODE_KEY,terminal?'terminal':'original');
  };

  const applyMotion=(value,save=false)=>{
    const off=value==='off';
    ...
    if(save) write(MOTION_KEY,off?'off':'on');
  };

  applyMode(read(MODE_KEY,'original'));
  applyMotion(read(MOTION_KEY,'on'));

  mode?.addEventListener('click',(e)=>{
    e.preventDefault();
    applyMode(body.classList.contains('terminal-mode')?'original':'terminal',true);
  });
  motion?.addEventListener('click',(e)=>{
    e.preventDefault();
    applyMotion(body.classList.contains('motion-off')?'on':'off',true);
  });
  document.addEventListener('click',(e)=>{
    const btn=e.target.closest('.try-terminal');
    if(!btn)return;
    e.preventDefault();
    applyMode(body.classList.contains('terminal-mode')?'original':'terminal',true);
    write('aliLabTerminalTried','1');
  });
});
